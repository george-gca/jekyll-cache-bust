...
1. **Build the gem** - This creates a `.gem` file from the current gemspec:
   ```bash
   gem build jekyll-cache-bust.gemspec
   ```

2. **Push to RubyGems** - This publishes the gem to [RubyGems.org](https://rubygems.org/), making it available for installation:
   ```bash
   gem push jekyll-cache-bust-X.X.X.gem
   ```
   Replace `X.X.X` with the actual version number (e.g., `jekyll-cache-bust-0.0.6.gem`).

The version number should be updated in [lib/jekyll-cache-bust/version.rb](lib/jekyll-cache-bust/version.rb) before building.
